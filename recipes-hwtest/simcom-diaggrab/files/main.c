...
static int create_log_file(const char *logfile_dir, const char *logfile_suffix, unsigned logfile_seq)
{
    ...
    snprintf(shortname, sizeof(shortname), "%s_%04d", time_name, logfile_seq);
...
_scan_tty:
    ...
    if(dev_dir){
        while ((ttyde = readdir(dev_dir)))
        {
            ...
            {
                snprintf(info.ttyDM, sizeof(info.ttyDM), "/dev/%s", ttyde->d_name);
                break;
            }
        }
        closedir(dev_dir);
...
__main_loop:
    ...
    time(&ltime);
    currtime = localtime(&ltime);
    snprintf((char *)diag_output_dir, sizeof(diag_output_dir),
